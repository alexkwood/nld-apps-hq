...
module.exports = function (options) {
  ...
  // * **ignore**: ignore missing .less files, don't throw error
  ...
  return function (req, res, next) {
    ...
    if (debug) log('relPath: ' + relPath + ', src: ' + src + ', dst: ' + dst);

    if (debug) log('request', dst + ' -> ' + src + (force ? ' (forced)' : ''));
    
    path.exists(src, function(exists) {
      if (!exists && ignore) {
        if (debug) log('LESS src ' + src + ' does not exist, ignoring.');
        return next();
      }

      if (force) return compile();

      // [checks if file is newer]
