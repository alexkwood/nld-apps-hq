...
everyauth.middleware = function () {
  var app = connect(
    ...
    , function fetchUserFromSession (req, res, next) {
        //console.log('** in fetchUserFromSession', req.session);
        var sess = req.session
          , auth = sess && sess.auth
          , everymodule, findUser;
        if (!auth) { console.log('no auth'); return next(); }
        if (!auth.userId) { console.log('no userId'); return next(); }
        everymodule = everyauth.everymodule;
        if (!everymodule._findUserById) { console.log('no userById'); return next(); }
        var pause = __pause(req);
        console.log('finding user by userId: ', auth.userId);
        everymodule._findUserById(auth.userId, function (err, user) {
          if (err) {
            console.log('error in _findUserById');
            throw err; // TODO Leverage everyauth's error handling
          }
          if (user) {
            req.user = user;
            ...
            console.log('deleting sess.auth');
            delete sess.auth;
          }

          console.log('done with fetchUserFromSession');
          next();
          pause.resume();
        });
      }
